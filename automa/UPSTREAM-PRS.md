...
|----|--------|----------|-------------------------------|
| [#8353](https://github.com/terraforming-mars/terraforming-mars/pull/8353) | `marsbot-postmerge-fixes` | Follow-ups to the merged #8206 and #8207: `Automa: X` card names to the `X:automa` suffix form (the client keeps what precedes the colon, so all eight rendered as "Automa"), `mcSupply`/`floaterCount` to `megacredits`/`floaters`, hook comments, and `MarsBotBoard` to `MarsBotTracks` with its array as `all`. **Round 1 addressed (2026-08-03):** the three `drawAndResolve*` methods return nothing and carry a `maybe` prefix, since no caller read the boolean. Answered without a change: the eight automa card names collide with nothing in the enum, and `tracks.all` stays because `tracks.tracks` stutters at every call site | Automa keeps the old `mcSupply`/`floaterCount` save keys and reads either |
| [#8209](https://github.com/terraforming-mars/terraforming-mars/pull/8209) | `marsbot-draft-resolver` | `MarsBotDraftResolver.ts` + spec. **Round 2 addressed (2026-08-03):** now a class taking tracks and an injected shuffle, `pickBest` made total, `postDraftDiscard` to `discardAfterDraft`, spec on real cards. **Round 4 addressed (2026-08-04):** kberg's "magic incantation" note on the tag score was right and led to two rule bugs. Tag priority is lexicographic on per-tag counts, not a weighted sum, so a card carrying the first priority tag now beats any number of lower ones (the old formula tied them, breaking Celestic, Aphrodite, EcoTec and Tycho Magnetics). The discard step discards the first card *without* a priority tag rather than everything before the first card with one, so at most one card leaves, and Credicor, Spire and Aridor now save what they drafted for. Also `scorer`/`score` rename, `hasIntersection`, and verb-form docs | Uses `MarsBotTracks`; this PR adopts the name when #8353 merges |
