...
| [#8210](https://github.com/terraforming-mars/terraforming-mars/pull/8210) | `marsbot-ma-eval` | `MarsBotMilestoneAwardEval.ts` + spec. **Round 2 addressed (2026-08-03):** `MarsBotMAContext` dropped, evals take `IMarsBot` and read the game like the real milestones. Fixed two bugs it exposed: Hydrologist counted owners of ocean tiles, which the engine never sets, so it could never be claimed; Visionary counted the Venus track twice. **Round 3 addressed (2026-08-06):** the 84-entry eval table is gone; the bot claims and scores through the ordinary `IMilestone`/`IAward` calls with its player (its tags and production read the tracks), and the four places where the automa rules differ (Planner, Banker, Thermalist, Miner) override new optional `marsBotCanClaim`/`marsBotScore` members, tested in their class specs. Fixes Terraformer under Turmoil: the class knows the 26 threshold where the table hardcoded 35 | `@/` imports -> relative; automa's `marsBotTrackPosition` reads the public `tagToTrack` record where upstream still has `getTrackIndexForTag` until #8353 lands; on automa the redesign also moves every non-Tharsis milestone and award to the classes, so Visionary now scores the bot's empty hand as 0 |
