...
| [#8208](https://github.com/terraforming-mars/terraforming-mars/pull/8208) | `marsbot-tile-placer` | `MarsBotTilePlacer.ts` + spec. **Round 2 addressed (2026-08-03):** `Board.ownedBy` filters, `drawOrThrow`, redundant `s.tile !== undefined` dropped, duplicated adjacency scorer collapsed, dead reserved-space check removed; spec rewritten so no test can pass without asserting, tiebreakers covered. **Round 3 addressed (2026-08-03):** `findExpediteConstructionCitySpace` renamed to match the Expedited Construction card, spec flattened to one `describe` per STYLE.md, `cast(x, undefined)` for undefined checks, assertions name the spaces they expect instead of counting neighbours, and two tests added for the greenery scoring rule. Pushed back on `maxOutOceans`: it stops at the 9-ocean cap, leaving 3 of Tharsis's 12 reserved spaces free | Spec setup: `testGame(2)` instead of `TestPlayer.RED.newPlayer` + `as any` cast |
