...
export class Executor implements BehaviorExecutor {
  public canExecute(behavior: Behavior, player: IPlayer, card: ICard, canAffordOptions?: CanAffordOptions) {
    ...
    if (behavior.global !== undefined) {
      ...
      if (g.temperature !== undefined && game.getTemperature() >= game.maxTemperature) {
        card.warnings.add('maxtemp');
      }
      if (g.oxygen !== undefined && game.getOxygenLevel() >= game.maxOxygenLevel) {
        if (g.oxygen < 0) {
      ...
      if (g.venus !== undefined && game.getVenusScaleLevel() >= game.maxVenusScale) {
        card.warnings.add('maxvenus');
      }
    }

    if (behavior.ocean !== undefined && game.board.getOceanSpaces().length >= game.maxOceanTiles) {
