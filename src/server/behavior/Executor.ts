  ...
  public canExecute(behavior: Behavior, player: IPlayer, card: ICard, canAffordOptions?: CanAffordOptions) {
    ...
    if (behavior.removeResourcesFromAnyCard !== undefined) {
      ...
      if (!(source !== 'self' && game.isSoloMode())) {
        ...
        // hasAvailableTargets, not getAvailableTargetCards: in an automa game MarsBot's
        // MC supply is a target even when no card holds the resource.
        if (RemoveResourcesFromCard.hasAvailableTargets(player, r.type, source, count) === false) {
          return false;
        }
      }
    }
