...
export class JetStreamMicroscrappers extends Card implements IActionCard {
  ...
  public canAct(player: IPlayer): boolean {
    const venusMaxed = player.game.getVenusScaleLevel() === player.game.maxVenusScale;
  ...
  public action(player: IPlayer) {
    ...
    if (this.resourceCount > 1 && player.game.getVenusScaleLevel() < player.game.maxVenusScale) {
