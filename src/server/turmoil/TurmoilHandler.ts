...
export class TurmoilHandler {
  ...
  public static computeTerraformRatingBump(player: IPlayer, tr: TRSource = {}): number {
    ...
    if (tr.oxygen !== undefined) {
      const availableSteps = player.game.maxOxygenLevel - player.game.getOxygenLevel();
    ...
    if (tr.temperature !== undefined) {
      const availableSteps = Math.floor((player.game.maxTemperature - player.game.getTemperature()) / 2);
    ...
    if (tr.oceans !== undefined) {
      const availableSteps = player.game.maxOceanTiles - player.game.board.getOceanSpaces().length;
      const steps = Math.min(availableSteps, tr.oceans);
      total = total + steps;
    }

    if (tr.venus !== undefined) {
      // |steps| counts fields, so derive available fields from the board's field list (the top of
      // the big board's track advances by 1% rather than 2%).
      const fields = player.game.board.venusFieldValues;
      const availableSteps = (fields.length - 1) - fields.indexOf(player.game.getVenusScaleLevel());
      const steps = Math.min(availableSteps, tr.venus);
      total = total + steps;
      if (player.game.getVenusScaleLevel() < constants.VENUS_LEVEL_FOR_TR_BONUS &&
        fields[fields.indexOf(player.game.getVenusScaleLevel()) + steps] >= constants.VENUS_LEVEL_FOR_TR_BONUS) {
        tr.tr = (tr.tr ?? 0) + 1;
      }
    }
