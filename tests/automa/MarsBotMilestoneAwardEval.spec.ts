...
describe('MarsBotMilestoneAwardEval', () => {
  ...
  describe('Integration', () => {
    it('Terraformer29 is filtered from milestones in automa game', () => {
      const {game} = createAutomaGame();
      const milestoneNames = game.milestones.map((m) => m.name);
      expect(milestoneNames).to.not.include('Terraformer29');
    });
