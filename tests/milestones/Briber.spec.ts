...
describe('Briber', () => {
  ...
  it('MarsBot qualifies with 20 M€', () => {
    expect(milestone.marsBotCanClaim({megacredits: 19} as unknown as IMarsBot)).is.false;
    expect(milestone.marsBotCanClaim({megacredits: 20} as unknown as IMarsBot)).is.true;
  });
});
