...
describe('Tactician', () => {
  ...
  it('MarsBot claims it with 35 M€', () => {
    expect(milestone.marsBotCanClaim({megacredits: 34} as unknown as IMarsBot)).is.false;
    expect(milestone.marsBotCanClaim({megacredits: 35} as unknown as IMarsBot)).is.true;
  });
});
