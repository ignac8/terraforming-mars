...
describe('Tactician4', () => {
  ...
  it('MarsBot qualifies with 30 M€', () => {
    expect(milestone.marsBotCanClaim({megacredits: 29} as unknown as IMarsBot)).is.false;
    expect(milestone.marsBotCanClaim({megacredits: 30} as unknown as IMarsBot)).is.true;
  });
});
